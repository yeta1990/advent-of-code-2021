...
//	char	*str = "D2FE28";
	...
	bits = 0;
	num_bin = 0;
	bits = malloc(sizeof(char) * 100000);
	aux = bits;
	bzero(bits, 100000);
	i = 0;
	while (*str)
...
int	parse_packet(char *str)
{
	...
	if (packet_version != 4)
	{
		...
		//	printf("\n%s\n", str);
			...
			while (i < subpackets)
			{
			//	printf("i %i, subpackets %i\n", i, subpackets);
				aux = str;
//				printf("->%i\n", parse_packet(aux));
	//			printf("str - aux %li\n", str - aux);
	//			str += str - aux;
				str += parse_packet(aux);
				i++;
			}
		}
		else
...
//			printf("---->%s\n", str);
			...
			str += 15;
	//		printf("i %i, subp_len %i\n", i, subp_len);
			while (i < subp_len)
			{
			/*	if (strnstr(str, "1", subp_len - i) == 0)
				{
				//	printf("->\n\n");
					i += subp_len - i;
					break ;	
...
//		printf("\n%s\n", str);
		...
		num = num << 4 | bin_to_int(str, 4);
		str += 4;
		printf("\nversion %i, packet type id %i, num %li\n", version, packet_version, num);
	}
//	printf("str - aux: %li\n", str - aux);
	return (str - aux);
}
