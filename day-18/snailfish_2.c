...
int	magnitude(t_pair *p)
{
	if (p && p->left && p->left->type == 0 && p->right && p->right->type == 0)
		return (p->left->num * 3 + p->right->num *2);
	else if (p && p->left && p->left->type == 1)
	{
		p->left->type = 0;
		p->left->num = magnitude(p->left->p);
		p->left->p = 0;
	}
	else if (p && p->right && p->right->type == 1)
	{
		p->right->type = 0;
		p->right->num = magnitude(p->right->p);
		p->right->p = 0;
	}
	return (magnitude(p));
}
